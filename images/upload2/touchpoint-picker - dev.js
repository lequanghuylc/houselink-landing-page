(async () => {
  ...
  if (
    !_apPath.endsWith('/wp-admin/post.php') &&
    !_apPath.endsWith('/wp-admin/post-new.php')
  )
    return;
  if (!document.body.classList.contains('post-type-touch_point')) return;
  console.log(window.location.pathname);
  const waitUntilElementsShow = (selectors) =>
    new Promise((resolve) => {
      const els = [];
  ...
  const els = await waitUntilElementsShow([
    '#pods-meta-touch-point',
    '#pods-form-ui-pods-meta-left',
    '#pods-form-ui-pods-meta-top',
    '#pods-form-ui-pods-meta-type-map',
    '#pods-form-ui-pods-meta-json',
  ]);
  await wait(300);
  ...
  if (!touchpointBox) {
    console.log('Can not find touchpointBox');
  }
  const inside = touchpointBox.querySelector('.inside');
  if (!inside) {
    console.log('Can not find inside');
  }
  ...
  const updateIframeUrl = () => {
    ...
    const siteSlug =
      typeof airportGetIframeSiteSlugPrefix === 'function'
        ? airportGetIframeSiteSlugPrefix()
        : '';
    const isDevDomain =
      window.location.hostname === 'cms.dev-amadeus-airport.devserver.london';

    if (iframe) {
      if (selectedValue === 'Inside') {
        iframe.src = `https://${isDevDomain ? 'dev-' : ''}amadeus-airport.devserver.london/${siteSlug}touchpoint-picker-pixi-inside${iframeUrlQuery}`;
      } else {
        iframe.src = `https://${isDevDomain ? 'dev-' : ''}amadeus-airport.devserver.london/${siteSlug}touchpoint-picker-pixi-outside${iframeUrlQuery}`;
      }
    }
  };
